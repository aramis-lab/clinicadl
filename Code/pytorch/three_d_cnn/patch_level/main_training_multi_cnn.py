    ...
    for i in range(options.num_cnn):
        ...
        for fi in range(options.n_splits):
            ...
            for epoch in range(options.epochs):
                ...
                # train the model
                train_subject, y_ground_train, y_hat_train, train_proba, acc_mean_train, global_step, loss_batch_mean = train(model, train_loader, use_cuda, loss, optimizer, writer_train_batch, epoch, fi, model_mode='train', global_step=global_step)
                ...
                valid_subject, y_ground_valid, y_hat_valid, valide_proba, acc_mean_valid, global_step, loss_batch_mean = train(model, valid_loader, use_cuda, loss, optimizer, writer_valid, epoch, fi, model_mode='valid', global_step=global_step)
