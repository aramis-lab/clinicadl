...
parser.add_argument("--output_dir", default='/teams/ARAMIS/PROJECTS/junhao.wen/PhD/ADNI_classification/gitlabs/AD-DL/Results/classifiers',
...
def main(options):
    ...
    for fi in range(options.n_splits):
        ...
        for epoch in range(options.epochs):
            ...
            # train the model
            train_subject, y_ground_train, y_hat_train, train_proba, acc_mean_train, global_step, loss_batch_mean_train = train(model, train_loader, options, loss, optimizer, writer_train_batch, epoch, fi, model_mode='train', global_step=global_step)

            ## calculate the accuracy with the whole training data for subject level balanced accuracy
            train_subject_all, y_ground_train_all, y_hat_train_all, train_proba_all, acc_mean_train_all, _, loss_batch_mean_train_all = train(model, train_loader, options, loss, optimizer, writer_train_all_data, epoch, fi, model_mode='valid', global_step=global_step)
            print("For training, subject level balanced accuracy is %f at the end of epoch %d" % (acc_mean_train_all, epoch))

            ## at then end of each epoch, we validate one time for the model with the validation data
            valid_subject, y_ground_valid, y_hat_valid, valide_proba, acc_mean_valid, global_step, loss_batch_mean_valid = train(model, valid_loader, options, loss, optimizer, writer_valid, epoch, fi, model_mode='valid', global_step=global_step)
            ...
            # save the best model based on the best acc and also save the last epoch model for training at stopping point
        ...
        ### calculate the final epoch performance for training and validation
