...
def adapt(old_tsv_dir: str, new_tsv_dir: str, subset_name="labels", labels_list="AD"):
    ...
    if not Path(old_tsv_dir).is_dir():
        raise ClinicaDLArgumentError(
            f"the directory: {old_tsv_dir} doesn't exists"
            "Please give an existing path"
        )
    if not Path(new_tsv_dir).is_dir():
        Path(new_tsv_dir).mkdir(parents=True, exist_ok=True)

    files_list = list(Path(old_tsv_dir).iterdir())

    df_baseline = pd.DataFrame()
    df_all = pd.DataFrame()

    for file in files_list:
        path = Path(old_tsv_dir) / file
        if path.is_file() and path.name.endswith(".tsv"):
            df_all, df_baseline = concat_files(path, df_baseline, df_all)

    if not df_all.empty:
        df_all.to_csv(Path(new_tsv_dir) / subset_name + ".tsv", sep="\t")
    if not df_baseline.empty:
        df_baseline.to_csv(Path(new_tsv_dir) / subset_name + "_baseline.tsv", sep="\t")
    ...
    if "kfold.json" in files_list:
        ...
        new_fold_dir = Path(new_tsv_dir) / (str(n_splits) + "_fold")
        new_fold_dir.mkdir(parents=True)

        for i in range(n_splits):
            new_kfold_dir = (
                Path(new_tsv_dir) / (str(n_splits) + "_fold") / ("split-" + str(i))
            )
