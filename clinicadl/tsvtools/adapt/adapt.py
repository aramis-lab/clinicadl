...
def adapt(old_tsv_dir : str, new_tsv_dir : str, subset_name="labels", labels_list="AD" ):
    ...
    if not df_baseline.empty:
        df_baseline.to_csv(os.path.join(new_tsv_dir, subset_name +"_baseline.tsv"), sep="\t")
    ...
    if "split.json" in files_list :
        ...
        adapt(os.path.join(old_tsv_dir, subset_name), new_split_dir, subset_name, labels_list)
    ...
    if "kfold.json" in files_list :
        ...
        for i in range(n_splits):
            new_kfold_dir = os.path.join(new_tsv_dir, str(n_splits) +"_fold", "split-" + str(i) )
            adapt(os.path.join(old_tsv_dir, "train_splits-"+str(n_splits), "split-" + str(i)), new_kfold_dir, "train", labels_list)
            adapt(os.path.join(old_tsv_dir, subset_name +"_splits-"+str(n_splits), "split-" + str(i)), new_kfold_dir, subset_name, labels_list)
