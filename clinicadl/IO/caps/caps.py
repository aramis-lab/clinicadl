...
class Caps(Directory):
    ...
    def load(self):
        ...
        for subject in self.subjects_list:
            subject_dir = SubjectDir.load(
                parent_path=self.subjects_dir, subject_id=subject
            )
            self.subjects[subject] = subject_dir

    @property
    def subjects_dir(self) -> Path:
        """Return the path to the subjects directory."""
        return self.path / SUBJECTS

    @property
    def dataset_description_json(self) -> Path:
        """Return the path to the dataset_description.json file."""
        return self.path / "dataset_description.json"
    ...
    def subjects_list(self) -> list[str]:
        ...
        return [
            x.name.split("-")[1]
            for x in self.subjects_dir.iterdir()
...
# bids = Bids(path=Path())
# bids.subjects[sub].sessions[ses].data[anat].t1w.json load()
...
class Files(ABC):
    ...
    def load(self, path: Path) -> None:
        ...
        for file in path.iterdir():
            if file.name.endswith(f"{subject}_{session}_{self.suffix}.json"):
                self.json = file
            if file.name.endswith(f"{subject}_{session}_{self.suffix}.nii*"):
                self.nii = file
...
class PETFiles(Files):
    def __init__(
        ...
        reconstruction: Union[str, ReconstructionMethod],
    ):
    ...
    def suffix(self) -> str:
        """Return the suffix of the image."""
        return f"trc-{self.tracer}_rec-{self.reconstruction}_pet"
