...
from clinicadl.networks.old_network.nn.layers import PadMaxPool2d, PadMaxPool3d
...
class RandomArchitecture(CNN):  # TODO : unabled to test it
    def __init__(
        self,
        convolutions_dict,
        n_fcblocks,
        ...
        for key, item in convolutions_dict.items():
            convolutional_block = self._define_convolutional_block(item)
            convolutions.add_module(key, convolutional_block)

        classifier = nn.Sequential(nn.Flatten(), nn.Dropout(p=dropout))

        fc, _ = self._fc_dict_design(
            n_fcblocks, convolutions_dict, input_size, output_size
        )
        for key, item in fc.items():
            n_fc = int(key[2::])
            if n_fc == len(fc) - 1:
                fc_block = self._define_fc_layer(item, last_block=True)
            else:
                fc_block = self._define_fc_layer(item, last_block=False)
            classifier.add_module(key, fc_block)

        super().__init__(convolution_layers=convolutions, fc_layers=classifier)

    def _define_convolutional_block(self, conv_dict):
        """
        ...
        for i in range(conv_dict["n_conv"] - 1):
            ...
            conv_block = self._append_normalization_layer(conv_block, in_channels)
        ...
        if conv_dict["d_reduction"] == "MaxPooling":
            ...
            conv_block = self._append_normalization_layer(conv_block, out_channels)
            conv_block.append(nn.LeakyReLU())
    ...
    def _append_normalization_layer(self, conv_block, num_features):
        """
    ...
    @staticmethod
    def _define_fc_layer(fc_dict, last_block=False):
        """
    ...
    @staticmethod
    def _fc_dict_design(n_fcblocks, convolutions, initial_shape, n_classes=2):
        """
