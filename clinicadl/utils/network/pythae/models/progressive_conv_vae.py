...
def build_encoder_decoder(
    ...
    enc_channels = [
        [input_c]
        + list(range(input_c + (first_layer_channels - input_c) // (n_conv_per_block), first_layer_channels, (first_layer_channels - input_c) // (n_conv_per_block)))[: n_conv_per_block - 1]
        + [first_layer_channels]
    ] + [
        [first_layer_channels * 2 ** (j)]
        + list(range(first_layer_channels * 2 ** (j) + (first_layer_channels * 2 ** (j+1) - first_layer_channels * 2 ** (j)) // (n_conv_per_block), first_layer_channels * 2 ** (j+1), (first_layer_channels * 2 ** (j+1) - first_layer_channels * 2 ** (j)) // (n_conv_per_block)))[: n_conv_per_block - 1]
        + [first_layer_channels * 2 ** (j+1)]
    for j in range(n_conv_encoder - 1)
    ]
    ...
    dec_channels = [
        [input_c]
        + list(range(input_c + (last_layer_channels - input_c) // (n_conv_per_block), last_layer_channels, (last_layer_channels - input_c) // (n_conv_per_block)))[: n_conv_per_block - 1]
        + [last_layer_channels]
    ] + [
        [last_layer_channels * 2 ** (j)]
        + list(range(last_layer_channels * 2 ** (j) + (last_layer_channels * 2 ** (j+1) - last_layer_channels * 2 ** (j)) // (n_conv_per_block), last_layer_channels * 2 ** (j+1), (last_layer_channels * 2 ** (j+1) - last_layer_channels * 2 ** (j)) // (n_conv_per_block)))[: n_conv_per_block - 1]
        + [last_layer_channels * 2 ** (j+1)]
    for j in range(n_conv_decoder - 1)
    ]
