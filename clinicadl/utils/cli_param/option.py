...
diagnoses = click.option(
    ...
    default=("AD", "CN"),
    help=(
        "Labels selected for the demographic analysis used in the context of Alzheimer's"
...
modality = click.option(
    ...
    type=str,
    help=(
        "Modality to select sessions. Sessions which do not include the modality will be"
        " excluded."
    ),
)
...
extract_json = click.option(
    ...
    default=None,
    help=(
        "Name of the JSON file created to describe the tensor extraction. "
...
use_uncropped_image = click.option(
    ...
    default=False,
    help=(
        "Use the uncropped image instead of the cropped image generated by t1-linear or"
        " pet-linear."
    ),
)
...
acq_label = click.option(
    ...
    help=(
        ...
        " fluorodeoxyglucose or '18FAV45' for florbetapir."
    ),
)
suvr_reference_region = click.option(
    ...
    help=(
        "Regions used for normalization if MODALITY is `pet-linear`. Intensity"
        " normalization using the average PET uptake in reference regions resulting in a"
        " standardized uptake value ratio (SUVR) map. It can be cerebellumPons or"
        " cerebellumPon2 (used for amyloid tracers) or pons or pons2 (used for 18F-FDG"
        " tracers)."
    ),
)
