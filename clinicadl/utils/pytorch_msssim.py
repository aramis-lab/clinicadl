...
class MSSSIM(torch.nn.Module):
    ...
    def ssim(self, img1: torch.Tensor, img2: torch.Tensor):
        ...
        luminance = torch.mean((2 * mu1 * mu2 + C1) / (mu1_sq + mu2_sq + C1))
        contrastxstructure = torch.mean((2.0 * sigma12 + C2) / (sigma1_sq + sigma2_sq + C2))

        return luminance, contrastxstructure
    ...
    def forward(self, y, y_pred):
        ...
        prod = torch.prod(mcs**weights)

        output = prod * (ml[-1]**weights[-1])
        return output