...
def generate_artifacts_dataset(
    ...
    def create_artifacts_image(data_idx, output_df):
        ...
        artif_image_nii_dir = (
            output_dir
            / "subjects"
            / f"sub-ARTIF{subject_id}"
            / session_id
            / preprocessing
        )
        artif_image_nii_filename = (
            f"sub-ARTIF{subject_id}_{session_id}_{filename_pattern}"
        )

        artif_image_nii_dir.mkdir(parents=True, exist_ok=True)

        artifacts = tio.RandomGamma(log_gamma=(gamma[0], gamma[1]))

        """
        artifacts = tio.transforms.Compose(
            [
                tio.RandomGamma(
                ...
                tio.RandomNoise(std=(noise_std[0], noise_std[1]),
                ),
            ]
        )
        """
        ...
        # Append row to output tsv
        row = [f"sub-ARTIF{subject_id}", session_id, "motion"]
        row_df = pd.DataFrame([row], columns=columns)
        output_df = pd.concat([output_df, row_df])
