...
def quality_check(caps_dir, output_dir, group_label):
    ...
    logger.info(f"Quality check metrics extracted at {path.join(output_dir, 'QC_metrics.tsv')}.")
    ...
    logger.info(
        f"Number of sessions removed based on frontal similarity with DARTEL template: {len(rejection2_df) - len(rejection3_df)}."
    )
