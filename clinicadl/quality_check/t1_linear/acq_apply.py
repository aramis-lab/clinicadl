...
if default_data_dir == "" or default_data_dir is None:
    default_data_dir = "."
...
def parse_options():
    ...
    parser = argparse.ArgumentParser(
        description="Apply automated QC",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )

    parser.add_argument(
        "--ref", action="store_true", default=False, help="Use reference images"
    )
    parser.add_argument(
        "--image", type=str, help="Input image prefix: <prefix>_{0,1,2}.jpg"
    )
    parser.add_argument(
        "--volume", type=str, help="Input minc volume (need minc2 simple)"
    )
    parser.add_argument(
        "--resample",
        type=str,
    ...
    parser.add_argument(
        "--low",
        type=float,
        help="Winsorize intensities in the input image",
        default=5.0,
    )
    parser.add_argument(
        "--high",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Load pretrained model (will try to  load from {}".format(
            default_data_dir
        ),
    )
    parser.add_argument(
        "--net",
        choices=["r18", "r34", "r50", "r101", "r152", "sq101"],
        help="Network type",
        default="r18",
    )
    parser.add_argument(
        "--raw", action="store_true", default=False, help="Print raw score [0:1]"
    )
    parser.add_argument(
        "-q",
        "--quiet",
    ...
    parser.add_argument(
        "--gpu", action="store_true", default=False, help="Run inference in gpu"
    )
    parser.add_argument(
        "--dist",
        action="store_true",
    ...
    with torch.no_grad():
        if params.batch is not None:
            if not params.batch_pics:
                dataset = MincVolumesDataset(
                    csv_file=params.batch,
            ...
            if params.image is not None:
                ...
                if params.resample is not None:
                    ...
                    try:
                        ...
                        subprocess.check_call(
                            args,
                            stdout=subprocess.DEVNULL if not params.debug else None,
                            stderr=subprocess.DEVNULL if not params.debug else None,
                        )
                    except:
                ...
                inputs = load_minc_images(
                    volume, winsorize_low=params.low, winsorize_high=params.high
                )
                ...
                try:
                    ...
                    inputs = load_talairach_mgh_images(
                        tmp_vol, winsorize_low=params.low, winsorize_high=params.high
                    )
                ...
                print(
                    "Specify input volume or image prefix or batch list, see help with --help",
                    file=sys.stderr,
                )
                exit(1)
            ...
            if use_ref:
                ...
                inputs = torch.cat(
                    [item for sublist in zip(inputs, ref_inputs) for item in sublist]
                ).unsqueeze_(0)
            else:
