...
from ..tools.deep_learning.autoencoder_utils import train, get_criterion, visualize_image
...
def resume_autoencoder(params, resumed_split):
    ...
    decoder, current_epoch = load_model(decoder, model_dir, params.gpu, 'checkpoint.pth.tar')
    ...
    if params.visualization:
        ...
        if nb_images <= 2:
            nb_images *= 3
        visualize_image(best_decoder, valid_loader, path.join(visualization_dir, "validation"),
                        nb_images=nb_images)
        visualize_image(best_decoder, train_loader, path.join(visualization_dir, "train"),
                        nb_images=nb_images)
    del decoder
