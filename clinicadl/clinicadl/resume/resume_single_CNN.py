...
from ..train.train_singleCNN import test_single_cnn
from ..tools.deep_learning.data import load_data, get_transforms, return_dataset, generate_sampler
...
from ..tools.deep_learning.iotools import return_logger, \
    commandline_to_json, write_requirements_version, translate_parameters
from ..tools.deep_learning.cnn_utils import train, get_criterion
...
def resume_single_cnn(params, resumed_split):
    ...
    model = init_model(params, initial_shape=data_train.size, len_atlas=data_train.len_atlas())
    model_dir = path.join(params.output_dir, f"fold-{resumed_split}", "models")
    model, current_epoch = load_model(model, model_dir, params.gpu, 'checkpoint.pth.tar')

    params.beginning_epoch = current_epoch + 1

    # Define criterion and optimizer
    criterion = get_criterion(params.loss)
    optimizer_path = path.join(params.output_dir, f"fold-{resumed_split}", "models", "optimizer.pth.tar")
    optimizer = load_optimizer(optimizer_path, model)

    # Define output directories
    log_dir = path.join(
        params.output_dir, f'fold-{resumed_split}', 'tensorboard_logs')
    model_dir = path.join(
        params.output_dir, f'fold-{resumed_split}', 'models')

    main_logger.debug('Beginning the training task')
    train(model, train_loader, valid_loader, criterion,
          optimizer, True, log_dir, model_dir, params, train_logger)

    test_single_cnn(model, params.output_dir, train_loader, "train",
                    resumed_split, criterion, params.mode, eval_logger, params.selection_threshold, gpu=params.gpu)
