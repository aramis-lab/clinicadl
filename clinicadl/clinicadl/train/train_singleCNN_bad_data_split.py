...
from clinicadl.tools.deep_learning.cnn_utils import mode_level_to_tsvs, test, train
...
parser.add_argument(
    "--epochs", default=1, type=int, help="Epochs through the data. (default=20)"
)

parser.add_argument(
    "--batch_size", default=32, type=int, help="Batch size for training. (default=1)"
)
...
parser.add_argument(
    "--weight_decay", default=1e-2, type=float, help="weight decay (default: 1e-4)"
)
...
def train_CNN_bad_data_split(params):
    ...
    print(f"The chosen network is {params.model}!")
    ...
    for fi in fold_iterator:
        print(f"Running for the {fi}-th fold")
        ...
        training_df, valid_df = mix_slices(
            training_sub_df, valid_sub_df, mri_plane=params.mri_plane
        )
    ...
    print(f"Total time of computation: {total_time} s")
