...
def retrain(new_options):
    ...
    if new_options.batch_size < options.batch_size:
        ...
        if not ratio.is_integer():
            warnings.warn(
                "The new batch size %i value is not a divisor of the previous one %i."
        ...
        if new_options.batch_size < options.batch_size * options.accumulation_steps:
            ...
            warnings.warn(
                "The previous batch size value was %i. "
    ...
    options.evaluation_steps = find_evaluation_steps(
        options.accumulation_steps, options.evaluation_steps
    )
