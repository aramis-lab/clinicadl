...
# Mandatory arguments
parser.add_argument("model_path", type=str, help="model selected")
parser.add_argument("split", type=int, help="Will load the specific split wanted.")
...
def main(options):
    ...
    if (
        options.evaluation_steps % options.accumulation_steps != 0
        and options.evaluation_steps != 1
    ):
        raise Exception(
    ...
    decoder, current_epoch = load_model(
        decoder, options.model_path, options.gpu, "checkpoint.pth.tar"
    )
    if options.gpu:
    ...
    log_dir = path.join(
        options.output_dir, "log_dir", "fold_%i" % options.split, "ConvAutoencoder"
    )
    visualization_dir = path.join(
        options.output_dir, "visualize", "fold_%i" % options.split
    )
    model_dir = path.join(
        options.output_dir,
        "best_model_dir",
