...
def main(options):
    ...
    model_dir = path.join(
        options.model_path, "best_model_dir", "CNN", "fold_" + str(options.split)
    )
    model, current_epoch = load_model(
        model, model_dir, options.gpu, "checkpoint.pth.tar"
    )
    ...
    model_dir = path.join(
        options.output_dir, "best_model_dir", "fold_%i" % options.split, "CNN"
    )
