...
def sampling_fn(value, sampling_type):
    if isinstance(value, (tuple, list)):
        if sampling_type is "fixed":
            ...
            return random_search_utils.choice(value)
        elif sampling_type is "exponent":
            exponent = random_search_utils.uniform(*value)
            return 10 ** -exponent
        elif sampling_type is "randint":
            return random_search_utils.randint(*value)
        elif sampling_type is "uniform":
            return random_search_utils.uniform(*value)
        else:
