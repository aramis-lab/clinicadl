...
import clinicadl.tools.deep_learning.models.modules as cdlnn
...
def sampling_fn(value, sampling_type):
    if isinstance(value, (tuple, list)):
        if sampling_type == "choice":
            ...
            raise ValueError(f"Sampling type {sampling_type} is not implemented")
    else:
...
def random_sampling(rs_options, options):
    ...
    additional_mode_dict = {
        ...
        "roi": {"selection_threshold": "uniform"},
        "slice": {
    ...
    if options.mode not in additional_mode_dict.keys():
        raise NotImplementedError(
            f"Mode {options.mode} was not correctly implemented for random search"
        )
...
class RandomArchitecture(nn.Module):
    ...
    def __init__(
        ...
        self.classifier = nn.Sequential(cdlnn.Flatten(), nn.Dropout(p=dropout))
    ...
    def return_layers_dict(self):
        if self.dimension == 3:
            ...
            layers = {
                "Conv": nn.Conv2d,
                "Pool": cdlnn.PadMaxPool2d,
                "InstanceNorm": nn.InstanceNorm2d,
                "BatchNorm": nn.BatchNorm2d,
            }
        else:
            raise ValueError(
                f"Cannot construct random network in dimension {self.dimension}"
            )
        return layers
    ...
    def cascading_randomization(self, n, random_model=None):
        ...
        if n > len(layers_list):
            raise ValueError(
                f"The number of randomized layers {n} cannot exceed the number of layers of the network {len(layers_list)}"
            )
        for i in range(-n, 0):
