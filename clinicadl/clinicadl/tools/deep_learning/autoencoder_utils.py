...
def visualize_image(decoder, dataloader, visualization_path, nb_images=1):
    ...
    for image_index in range(nb_images):
        ...
        nib.save(
            output_nii,
            os.path.join(visualization_path, f"output-{image_index}.nii.gz"),
        )
        nib.save(
            input_nii, os.path.join(visualization_path, f"input-{image_index}.nii.gz")
        )
...
def get_criterion(option):
    ...
    if option == "default":
        ...
        raise ValueError(f"The option {option} is unknown for criterion selection")
