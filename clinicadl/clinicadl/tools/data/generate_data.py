...
def generate_trivial_dataset(
    ...
    for i in range(2 * n_subjects):
        ...
        image_path = find_image_path(
            caps_dir, participant_id, session_id, preprocessing
        )
        image_nii = nib.load(image_path)
        image = image_nii.get_data()
...
def generate_shepplogan_dataset(
    ...
    for idx_label, label in enumerate(labels_distribution.keys()):
        samples_per_subtype = np.array(labels_distribution[label]) * samples
        for subtype in range(len(samples_per_subtype)):
            for idx_sample in range(int(samples_per_subtype[subtype])):
                participant_id = "sub-CLNC%i%04d" % (
                    idx_label,
                    idx_sample + np.sum(samples_per_subtype[:subtype:]).astype(int),
                )
                session_id = "ses-M00"
                row_df = pd.DataFrame(
