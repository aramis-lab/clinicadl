...
def main(options):
    ...
    source_data = CAPSVoxelBasedInput(
        options.caps_directory_source,
        diagnosis_tsv_all,
        options.group_id_source,
        "T1",
        fwhm=8,
        mask_zeros=False,
    )
    _, source_orig_shape, source_mask = load_data_svm(
        source_data.get_images(), mask=True
    )
    ...
    for fi in range(options.n_splits):
        ...
        if options.set == "validation":
            _, test_df = load_data(
                options.diagnosis_path, options.diagnoses, fi, options.n_splits, True
            )
        else:
            test_df = load_data_test(options.diagnosis_path, options.diagnoses)

        test_path = save_data(
            test_df, options.output_dir, path.join("fold_%i" % fi, folder_name)
        )

        target_data = CAPSVoxelBasedInput(
            options.caps_directory_target,
            test_path,
            options.group_id_target,
            "T1",
            fwhm=8,
            mask_zeros=False,
        )
        ...
        subjects_sessions = [
            subjects[i] + "_" + sessions[i] for i in range(len(subjects))
        ]

        weights = np.loadtxt(
            path.join(
                options.output_dir, "classifier", "fold_" + str(fi), "weights.txt"
            )
        )
        ...
        b = np.loadtxt(
            path.join(
                options.output_dir, "classifier", "fold_" + str(fi), "intersect.txt"
            )
        )
        ...
        subjects_df = pd.DataFrame(
            {
                "subject": np.array(subjects_sessions),
                "y": target_label,
                "y_hat": y_binary,
                "fold_index": fi,
            }
        ...
        subjects_df.to_csv(
            path.join(result_dir, "subjects_fold-" + str(fi) + ".tsv"),
            index=False,
            sep="\t",
            encoding="utf-8",
        )
