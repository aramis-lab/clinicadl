...
class CAPSInput(base.MLInput):
    def __init__(
        ...
        if list(subjects_visits.columns.values) != [
            "participant_id",
            "session_id",
            "diagnosis",
        ]:
...
class CAPSVoxelBasedInput(CAPSInput):
    def __init__(
        ...
        super(CAPSVoxelBasedInput, self).__init__(
            caps_directory,
            diagnoses_tsv,
            group_id,
            image_type,
    ...
    def get_x(self):
        ...
        self._x, self._orig_shape, self._data_mask = vbio.load_data(
            self._images, mask=self._mask_zeros
        )
    ...
    def save_results(self, output_dir):
        ...
        for i in range(len(self._fold_results)):
            ...
            results_df = pd.DataFrame(
                {
                    "balanced_accuracy": self._fold_results[i]["evaluation"][
                        "balanced_accuracy"
                    ],
...
def extract_indices_from_5_fold(
    diagnosis_tsv_folder,
    n_splits,
    output_dir,
    baseline=True,
    ...
    for i in range(n_splits):
        ...
        train_path = path.join(
            diagnosis_tsv_folder, "train_splits-" + str(n_splits), "split-" + str(i)
        )
        valid_path = path.join(
            diagnosis_tsv_folder,
            "validation_splits-" + str(n_splits),
            "split-" + str(i),
        )
        ...
        for j in range(len(train_df)):
            ...
            for index, row_all in all_df.iterrows():
                if (
                    row["participant_id"] == row_all["participant_id"]
                    and row["session_id"] == row_all["session_id"]
                    and row["diagnosis"] == row_all["diagnosis"]
                ):
...
def revert_mask(weights, mask, shape):
    ...
    z[
        mask
    ] = weights  # ValueError: NumPy boolean array indexing assignment cannot assign 1636161 input values to the 1634188 output values where the mask is true
...
def apply_best_parameters_each_split(
    kernel, x, y, results_list, balanced, n_fold, diagnoses_tsv, output_dir
):
    """
