...
def get_caps_t1(caps_directory, tsv):
    ...
    for i in range(len(participant_id)):
        ...
        cropped_hipp_file_name.append(
            participant_id[i]
            + "_"
            + session_id[i]
            + "_space-MNI_res-1x1x1_hippocampus.nii"
        )
...
def save_as_pt(input_img):
    ...
    output_file = os.path.join(
        os.path.dirname(input_img), input_img.split(".nii")[0] + ".pt"
    )
    # save
...
def get_subid_sesid_datasink(participant_id, session_id, caps_directory, hemi):
    ...
    base_directory = os.path.join(
        caps_directory, "subjects", participant_id, session_id, "t1", "preprocessing_dl"
    )
    ...
    subst_tuple_list = [
        (
            ...
            + "_"
            + session_id
            + "_space-MNI_res-1x1x1_hippocampus.nii.gz",
            participant_id
            + "_"
            + session_id
            + "_space-MNI_res-1x1x1_hippocampus_hemi-"
            + hemi
            + ".nii.gz",
        ),
        (
