...
def compute_abnormality_map(
    ...
    n_proc: int,
    batch_size: int,
    abn_map_fn: Union[str, Callable[[torch.Tensor, torch.Tensor], Tuple[torch.Tensor, str]]] = residual_fn,
    ...
    Args:
        ...
        abn_map_fn (Callable[[torch.Tensor, torch.Tensor], Tuple[torch.Tensor, str]], optional): 
    ...
    for split in split_list:
        ...
        input_data = return_dataset(
            Path(maps_dir, f"split-{split}", f"best-{split_selection_metrics}", "CapsOutput"),
            group_df,
        ...
        for i, (input_batch, output_batch) in enumerate(zip(input_loader, output_loader)):
            ...
            for idx in range(len(input_batch['participant_id'])):
                ...
                if type(abn_map_fn) == str:
                    abn_map_fn = abn_map_dict[abn_map_fn]
                
                abn_map, abn_map_pattern = abn_map_fn(output_batch['data'][idx][0], input_batch['data'][idx][0])
                abn_map_path = input_batch['image_path'][idx].replace("_input.pt", f"_{abn_map_pattern}.pt")
                torch.save(abn_map, abn_map_path)
                print("Saved abnormality map at", abn_map_path)
