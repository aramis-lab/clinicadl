...
def test_interpret(cli_commands):
    ...
    train_error = not os.system("clinicaaddl " + " ".join(cnn_input))
    interpret_error = not os.system("clinicaaddl " + " ".join(interpret_input))
    interpret_flag = os.path.exists(
        os.path.join("results", "fold-0", "gradients"))
    assert train_error
