...
a path to a :term:`NIfTI` file, or a 4D :py:class:`torch.Tensor` (in which case the affine
...
.. tip::
    ...
    through the entire ClinicaDL pipeline.
...
:doc:`Reading BIDS datasets <bids>` in the following page): when a dataset loads an image, or extracts a
patch or a slice from it, it returns a ``Sample``.

.. tip::
    A ``Sample`` is a ``Datapoint``, so what has been mentioned in the :ref:`previous section <datapoint>` also applies to a ``Sample``.

In addition to the ``DataPoint`` fields, a ``Sample`` carries:

- ``file_type``: the :py:class:`~clinicadl.io.bids.BidsFileType` describing the loaded file;
- ``image_path``: the path to the loaded image;
...
.. code-block:: python
    ...
    >>> slice_sample = Colin27Sample2D(squeeze=False)
    ...
    108
    >>> slice_sample.get_image_tensor("image").shape
    torch.Size([1, 181, 1, 181])
...
.. code-block:: python
    ...
    >>> slice_sample = Colin27Sample2D(squeeze=True)
    >>> slice_sample.get_image_tensor("image").shape
    torch.Size([1, 181, 181])

----

Now that you know the objects ClinicaDL uses to carry your data, the next section
shows how ClinicaDL produces these ``Samples`` from :term:`BIDS` directories.
