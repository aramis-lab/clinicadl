...
A :py:class:`~clinicadl.train.Trainer` runs a fixed sequence of steps, but at many
...
.. important::
    ...
    Callbacks capture **non-essential** logic (logging, checkpointing, …). The
...
batch, the moment metrics are computed, …). You attach callbacks to a ``Trainer``
...
.. list-table::
    ...
    * - :py:class:`~clinicadl.callbacks.LRSchedulerCallback`
      - Adjusts the learning rate during training (wraps a PyTorch scheduler or an
        :py:mod:`lr-scheduler config <clinicadl.optim.lr_schedulers.config>`).
...
customise the loop. The :doc:`next chapter <../reproducibility/index>` explains how
