...
To split a :py:class:`~clinicadl.data.datasets.ConcatDataset` (or its derivatives:
...
To find a split, you may be interested in:
...
Then, to split a dataset between a training and a validation sets, you will need to read the splits you made with ``make_split``
